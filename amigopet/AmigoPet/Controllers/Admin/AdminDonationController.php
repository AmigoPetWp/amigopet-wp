...
class AdminDonationController extends BaseAdminController
{
    ...
    {
        $this->checkPermission('manage_amigopet_donations');
        check_admin_referer('apwp_save_donation');
        ...
        try {
            if ($id) {
                ...
                $message = esc_html__('Doação atualizada com sucesso!', 'amigopet');
            } else {
                $this->donationService->createDonation($data);
                $message = esc_html__('Doação cadastrada com sucesso!', 'amigopet');
            }

            $this->redirect(
                admin_url('admin.php?page=amigopet-donations'),
                $message
            );
        } catch (\Exception $e) {
            $this->redirect(
    ...
    {
        $this->checkPermission('manage_amigopet_donations');
        check_admin_referer('apwp_process_payment');

        $id = isset($_POST['id']) ? (int) $_POST['id'] : 0;
        $transaction_id = sanitize_text_field($_POST['transaction_id']);

        if (!$id || !$transaction_id) {
            wp_die(esc_html__('ID da doação e ID da transação são obrigatórios', 'amigopet'));
        }
        ...
        try {
            ...
            $this->redirect(
                admin_url('admin.php?page=amigopet-donations'),
                esc_html__('Pagamento processado com sucesso!', 'amigopet')
            );
        } catch (\Exception $e) {
            $this->redirect(
                admin_url('admin.php?page=amigopet-donations'),
    ...
    {
        $this->checkPermission('manage_amigopet_donations');
        check_admin_referer('apwp_refund_donation');

        $id = isset($_POST['id']) ? (int) $_POST['id'] : 0;
        if (!$id) {
            wp_die(esc_html__('ID da doação não fornecido', 'amigopet'));
        }
        ...
        try {
            ...
            $this->redirect(
                admin_url('admin.php?page=amigopet-donations'),
                esc_html__('Doação reembolsada com sucesso!', 'amigopet')
            );
        } catch (\Exception $e) {
            $this->redirect(
                admin_url('admin.php?page=amigopet-donations'),
    ...
    {
        $this->checkPermission('manage_amigopet_donations');
        check_admin_referer('apwp_mark_failed');
        ...
        try {
            ...
            $this->redirect(
                admin_url('admin.php?page=amigopet-donations'),
                esc_html__('Doação marcada como falha com sucesso!', 'amigopet')
            );
        } catch (\Exception $e) {
            $this->redirect(
                admin_url('admin.php?page=amigopet-donations'),
