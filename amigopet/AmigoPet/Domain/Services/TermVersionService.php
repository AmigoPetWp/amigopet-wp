...
class TermVersionService {
    ...
    public function createVersion(array $data): int {
        ...
        if ($this->repository->findByVersion($data['term_id'], $data['version'])) {
            throw new \InvalidArgumentException(esc_html__('Já existe uma versão para este termo', 'amigopet'));
        }
    ...
    public function updateVersion(int $id, array $data): void {
        ...
        if (!$version) {
            throw new \InvalidArgumentException(esc_html__("Versão não encontrada", 'amigopet'));
        }

        if (!$version->isDraft()) {
            throw new \InvalidArgumentException(esc_html__("Apenas versões em rascunho podem ser atualizadas", 'amigopet'));
        }
    ...
    public function deleteVersion(int $id): void {
        if (!$this->repository->delete($id)) {
            throw new \RuntimeException(esc_html__("Erro ao excluir a versão", 'amigopet'));
        }
