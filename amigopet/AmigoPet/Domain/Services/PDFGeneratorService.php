...
class PDFGeneratorService {
    ...
    public function signPDF(string $filePath, string $certificate, string $password): bool {
        ...
        if (!file_exists($filePath)) {
            throw new \InvalidArgumentException(esc_html__("Arquivo PDF não encontrado", 'amigopet'));
        }

        // Verifica se o certificado existe
        if (!file_exists($certificate)) {
            throw new \InvalidArgumentException(esc_html__("Certificado digital não encontrado", 'amigopet'));
        }
        ...
        try {
            ...
            throw new \RuntimeException(
                sprintf(
                    ...
                    esc_html__('Erro ao assinar o PDF: %s', 'amigopet'),
    ...
    public function protectPDF(string $filePath, string $userPassword, string $ownerPassword): bool {
        ...
        try {
            ...
            throw new \RuntimeException(
                sprintf(
                    /* translators: %s: original error message */
                    esc_html__('Erro ao proteger o PDF: %s', 'amigopet'),
                    esc_html(sanitize_text_field($e->getMessage()))
                )
            );
        }
    ...
    public function addWatermark(string $filePath, string $text): bool {
        ...
        try {
            ...
            throw new \RuntimeException(
                sprintf(
                    /* translators: %s: original error message */
                    esc_html__('Erro ao adicionar marca d\'agua ao PDF: %s', 'amigopet'),
                    esc_html(sanitize_text_field($e->getMessage()))
                )
            );
        }
