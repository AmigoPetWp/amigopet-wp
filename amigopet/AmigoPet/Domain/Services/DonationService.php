...
class DonationService {
    ...
    public function processPayment(int $donationId, string $transactionId): void {
        ...
        if (!$donation) {
            throw new \InvalidArgumentException(esc_html__("Doação não encontrada", 'amigopet'));
        }
    ...
    public function refundPayment(int $donationId): void {
        ...
        if ($donation->getPaymentStatus() !== Donation::STATUS_COMPLETED) {
            throw new \InvalidArgumentException(esc_html__("Apenas doações pagas podem ser reembolsadas", 'amigopet'));
        }
