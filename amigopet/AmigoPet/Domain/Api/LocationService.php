...
class LocationService
{
    ...
    public function geocode(string $address): array
    {
        ...
        if (is_wp_error($response)) {
            ...
            throw new \RuntimeException(esc_html(sprintf(esc_html__('Erro na geocodificação: %s', 'amigopet'), esc_html($response->get_error_message()))));
        }

        $result = json_decode(wp_remote_retrieve_body($response), true);
        if (!$result || $result['status'] !== 'OK') {
            throw new \RuntimeException(esc_html__('Erro ao geocodificar endereço', 'amigopet'));
        }
    ...
    public function reverseGeocode(float $lat, float $lng): array
    {
        ...
        if (is_wp_error($response)) {
            // translators: %s: placeholder
            throw new \RuntimeException(esc_html(sprintf(esc_html__('Erro na geocodificação reversa: %s', 'amigopet'), esc_html($response->get_error_message()))));
        }

        $result = json_decode(wp_remote_retrieve_body($response), true);
        if (!$result || $result['status'] !== 'OK') {
            throw new \RuntimeException(esc_html__('Erro ao geocodificar coordenadas', 'amigopet'));
        }
