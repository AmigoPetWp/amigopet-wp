...
class AdoptionDocument {
    ...
    private function validate(): void {
        if (!array_key_exists($this->documentType, self::ALLOWED_DOCUMENT_TYPES)) {
            throw new \InvalidArgumentException(esc_html__("Tipo de documento inválido", 'amigopet'));
        }
        ...
        if (empty($this->content)) {
            throw new \InvalidArgumentException(esc_html__("Conteúdo do documento é obrigatório", 'amigopet'));
        }

        if ($this->signedBy <= 0) {
            throw new \InvalidArgumentException(esc_html__("ID do assinante inválido", 'amigopet'));
        }

        if (!filter_var($this->documentUrl, FILTER_VALIDATE_URL)) {
            throw new \InvalidArgumentException(esc_html__("URL do documento inválida", 'amigopet'));
        }
    }
    ...
    public function sign(): void {
        if (!$this->isPending()) {
            throw new \InvalidArgumentException(esc_html__("Documento não está pendente de assinatura", 'amigopet'));
        }
        $this->status = 'signed';
        $this->signedAt = new \DateTime();
        $this->updatedAt = new \DateTime();
    ...
    public function cancel(): void {
        if ($this->isSigned()) {
            throw new \InvalidArgumentException(esc_html__("Documento já foi assinado", 'amigopet'));
        }
    ...
    public function expire(): void {
        if ($this->isSigned() || $this->isCancelled()) {
            throw new \InvalidArgumentException(esc_html__("Não é possível expirar um documento assinado ou cancelado", 'amigopet'));
        }
        $this->status = 'expired';
        $this->updatedAt = new \DateTime();
    }
