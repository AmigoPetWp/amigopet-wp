...
class Adoption
{
    ...
    private function validate(): void
    {
        if (empty($this->adoptionReason)) {
            throw new \InvalidArgumentException(esc_html__("Motivo da adoção é obrigatório", 'amigopet'));
        }

        if (empty($this->petExperience)) {
            throw new \InvalidArgumentException(esc_html__("Experiência com pets é obrigatória", 'amigopet'));
        }
    }
    ...
    public function review(int $reviewerId, string $reviewNotes, bool $approved): void
    {
        if ($this->status !== 'pending' && $this->status !== 'rejected') {
            throw new \InvalidArgumentException(esc_html__("Adoção não está em estado para revisão", 'amigopet'));
        }
    ...
    public function markAsAwaitingPayment(): void
    {
        if ($this->status !== 'approved') {
            throw new \InvalidArgumentException(esc_html__("Adoção precisa estar aprovada", 'amigopet'));
        }
    ...
    public function markAsPaid(): void
    {
        if ($this->status !== 'awaiting_payment') {
            throw new \InvalidArgumentException(esc_html__("Adoção precisa estar aguardando pagamento", 'amigopet'));
        }
        $this->status = 'paid';
    }
    ...
    {
        if ($this->status !== 'paid') {
            throw new \InvalidArgumentException(esc_html__("Adoção precisa estar paga", 'amigopet'));
        }
    ...
    public function cancel(): void
    {
        if (!in_array($this->status, ['pending', 'approved', 'awaiting_payment'])) {
            throw new \InvalidArgumentException(esc_html__("Adoção não pode ser cancelada no status atual", 'amigopet'));
        }
