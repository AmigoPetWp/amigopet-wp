...
class AdoptionPayment {
    ...
    private function validate(): void {
        if ($this->amount <= 0) {
            throw new \InvalidArgumentException(esc_html__("O valor do pagamento deve ser maior que zero", 'amigopet'));
        }
        ...
        if ($this->payerEmail && !filter_var($this->payerEmail, FILTER_VALIDATE_EMAIL)) {
            throw new \InvalidArgumentException(esc_html__("Email do pagador inválido", 'amigopet'));
        }
    }

    public function startProcessing(?string $transactionId = null): void {
        if ($this->status !== self::STATUS_PENDING) {
            throw new \InvalidArgumentException(esc_html__("Apenas pagamentos pendentes podem ser processados", 'amigopet'));
        }
    ...
    public function confirmPayment(?string $transactionId = null): void {
        if ($this->status !== self::STATUS_PENDING && $this->status !== self::STATUS_PROCESSING) {
            throw new \InvalidArgumentException(esc_html__("Pagamento não pode ser confirmado no status atual", 'amigopet'));
        }
    ...
    public function markAsFailed(?string $reason = null): void {
        if ($this->status === self::STATUS_COMPLETED || $this->status === self::STATUS_REFUNDED) {
            throw new \InvalidArgumentException(esc_html__("Pagamento não pode ser marcado como falho no status atual", 'amigopet'));
        }
    ...
    public function refundPayment(?string $reason = null): void {
        if ($this->status !== self::STATUS_COMPLETED) {
            throw new \InvalidArgumentException(esc_html__("Apenas pagamentos concluídos podem ser reembolsados", 'amigopet'));
        }
    ...
    public function cancel(?string $reason = null): void {
        if ($this->status === self::STATUS_COMPLETED || $this->status === self::STATUS_REFUNDED) {
            throw new \InvalidArgumentException(esc_html__("Pagamento não pode ser cancelado no status atual", 'amigopet'));
        }
