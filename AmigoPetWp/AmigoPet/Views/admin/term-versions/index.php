    ...
    <?php if (!$termId): ?>
        ...
        <?php else: ?>
            <table class="wp-list-table widefat fixed striped">
                ...
                <tbody>
                    <?php foreach ($pendingReview as $version): ?>
                        <tr>
                            <td><?php echo esc_html($version->getTermName()); ?></td>
                            <td><?php echo esc_html($version->getVersion()); ?></td>
                            <td><?php echo esc_html(get_userdata($version->getCreatedBy())->display_name); ?></td>
                            <td><?php echo esc_html($version->getEffectiveDate()->format('d/m/Y')); ?></td>
                            <td>
                                <a href="#" class="apwp-preview-version" data-id="<?php echo esc_attr($version->getId()); ?>">Visualizar</a>
                                <?php if (current_user_can('approve_terms')): ?>
                                    | <a href="#" class="apwp-approve-version" data-id="<?php echo esc_attr($version->getId()); ?>">Aprovar</a>
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
        ...
        <div class="apwp-term-versions-container">
                    ...
                    <table class="wp-list-table widefat fixed striped">
                        ...
                        <tbody>
                            <?php foreach ($versions as $version): ?>
                                <tr class="<?php echo $version->isActive() ? 'active-version' : ''; ?>">
                                    <td><?php echo esc_html($version->getVersion()); ?></td>
                                    <td><?php echo esc_html($version->getStatusName()); ?></td>
                                    <td><?php echo esc_html(get_userdata($version->getCreatedBy())->display_name); ?></td>
                                    <td><?php echo esc_html($version->getEffectiveDate()->format('d/m/Y')); ?></td>
                                    <td>
                                        <a href="#" class="apwp-preview-version" data-id="<?php echo esc_attr($version->getId()); ?>">Visualizar</a>
                                        <?php if ($version->isDraft()): ?>
                                            | <a href="#" class="apwp-edit-version" data-id="<?php echo esc_attr($version->getId()); ?>">Editar</a>
                                            | <a href="#" class="apwp-send-to-review" data-id="<?php echo esc_attr($version->getId()); ?>">Enviar para Revisão</a>
                                            <?php if (current_user_can('delete_terms')): ?>
                                                | <a href="#" class="apwp-delete-version" data-id="<?php echo esc_attr($version->getId()); ?>">Excluir</a>
                                            <?php endif; ?>
                                        <?php elseif ($version->isInReview() && current_user_can('approve_terms')): ?>
                                            | <a href="#" class="apwp-approve-version" data-id="<?php echo esc_attr($version->getId()); ?>">Aprovar</a>
                                        <?php elseif ($version->isApproved() && current_user_can('activate_terms')): ?>
                                            | <a href="#" class="apwp-activate-version" data-id="<?php echo esc_attr($version->getId()); ?>">Ativar</a>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
            ...
            <div id="apwp-version-form" style="display: none;">
                ...
                <form method="post">
                    ...
                    <input type="hidden" name="term_id" value="<?php echo esc_attr($termId); ?>">
                    <input type="hidden" name="id" value="">
                    <?php wp_nonce_field('apwp_term_versions'); ?>
