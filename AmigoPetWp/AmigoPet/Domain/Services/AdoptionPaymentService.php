...
class AdoptionPaymentService {
    ...
    public function validate(array $data): array {
        ...
        if (empty($data['adoption_id'])) {
            $errors['adoption_id'] = __('Adoção é obrigatória', 'amigopet-wp');
        } elseif (!$this->adoptionRepository->findById($data['adoption_id'])) {
            $errors['adoption_id'] = __('Adoção não encontrada', 'amigopet-wp');
        }

        if (!isset($data['amount'])) {
            $errors['amount'] = __('Valor é obrigatório', 'amigopet-wp');
        } elseif (!is_numeric($data['amount']) || $data['amount'] <= 0) {
            $errors['amount'] = __('Valor inválido', 'amigopet-wp');
        }

        if (empty($data['payment_method'])) {
            $errors['payment_method'] = __('Método de pagamento é obrigatório', 'amigopet-wp');
        } elseif (!in_array($data['payment_method'], ['cash', 'credit_card', 'debit_card', 'bank_transfer', 'pix'])) {
            $errors['payment_method'] = __('Método de pagamento inválido', 'amigopet-wp');
        }
