...
class SignedTermService {
    ...
    public function validate(array $data): array {
        ...
        if (empty($data['term_id'])) {
            $errors['term_id'] = __('Termo é obrigatório', 'amigopet-wp');
        } elseif (!$this->termRepository->findById($data['term_id'])) {
            $errors['term_id'] = __('Termo não encontrado', 'amigopet-wp');
        }

        if (empty($data['user_id'])) {
            $errors['user_id'] = __('Usuário é obrigatório', 'amigopet-wp');
        } elseif (!get_user_by('ID', $data['user_id'])) {
            $errors['user_id'] = __('Usuário não encontrado', 'amigopet-wp');
        }

        if (empty($data['ip_address'])) {
            $errors['ip_address'] = __('Endereço IP é obrigatório', 'amigopet-wp');
        }

        if (empty($data['user_agent'])) {
            $errors['user_agent'] = __('User Agent é obrigatório', 'amigopet-wp');
        }
