...
class TermTypeService {
    ...
    public function validate(array $data): array {
        ...
        if (isset($data['slug'])) {
            if (empty($data['slug'])) {
                $errors['slug'] = __('Slug é obrigatório', 'amigopet-wp');
            } elseif (!preg_match('/^[a-z0-9-]+$/', $data['slug'])) {
                $errors['slug'] = __('Slug inválido. Use apenas letras minúsculas, números e hífens', 'amigopet-wp');
            }
        }

        if (isset($data['required']) && !is_bool($data['required'])) {
            $errors['required'] = __('Campo obrigatório deve ser um booleano', 'amigopet-wp');
        }
