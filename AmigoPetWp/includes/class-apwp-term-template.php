...
/**
 * Classe responsável pelo gerenciamento de templates de termo personalizados
 *
 * @since      1.0.0
 * @package    AmigoPetWp
...
class APWP_Term_Template {
    ...
    /**
     * Lista de palavras mágicas obrigatórias no termo
     *
     * @since    1.0.0
     * @var      array
     */
    ...
    /**
     * Lista de tags dinâmicas para substituição no termo
     *
     * @since    1.0.0
     * @var      array
     */
    ...
    public function upload_template($file) {
        ...
        if ($movefile && !isset($movefile['error'])) {
            ...
            if ($validation['is_valid']) {
                ...
                update_option('amigopet_term_template', $movefile['file']);
    ...
    public function replace_dynamic_tags($content, $data) {
        ...
        // Dados do Termo
        $content = str_replace('[[DATA_TERMO]]', $data['date'] ?? date('d/m/Y'), $content);
        $content = str_replace('[[NUMERO_TERMO]]', $data['term_number'] ?? '', $content);
    ...
    /**
     * Processa o upload de um novo template de termo
     *
     * @since    1.0.0
    ...
    /**
     * Substitui tags no template de termo
     *
     * @since    1.0.0
    ...
    public function convert_to_pdf($content, $data) {
        ...
        $pdf->SetTitle('Termo de Adoção');
        $pdf->AddPage();

        // Adiciona o conteúdo ao PDF
        $pdf->writeHTML($content, true, false, true, false, '');

        // Define o diretório para salvar os termos
        $upload_dir = wp_upload_dir();
        $terms_dir = $upload_dir['basedir'] . '/apwp-terms';
    ...
    /**
     * Gera termo com template personalizado
     *
     * @since    1.0.0
     * @param    array    $data    Dados do termo
     * @return   string            Caminho do PDF gerado
     */
    public function generate_custom_term($data) {
        // Recupera o template salvo
        $template_path = get_option('amigopet_term_template');

        if (!$template_path || !file_exists($template_path)) {
            // Usa o template padrão se nenhum for encontrado
            return (new APWP_Term())->generate($data);
        }
        ...
        if (!$tag_validation['is_valid']) {
            return new WP_Error('invalid_template', 'O modelo de termo não contém todas as tags necessárias', [
